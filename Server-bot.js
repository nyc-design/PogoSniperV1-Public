...
async function sendNotification(lat, lng, pokemonName) { const latF = Number(lat); const lngF = Number(lng); if (Number.isNaN(latF) || Number.isNaN(lngF)) return logWarn('[NTFY] Invalid coords; skipping.'); if (GEOFENCE_CENTER && GEOFENCE_RADIUS_KM > 0 && validateCoordPair(GEOFENCE_CENTER)) { const dist = haversineDistance(GEOFENCE_CENTER, [latF, lngF]); if (dist > GEOFENCE_RADIUS_KM) return logInfo(`[FOUND] ${latF.toFixed(5)},${lngF.toFixed(5)} → Outside geofence. Not sent.`); } const title = pokemonName ? `${pokemonName} Found!` : 'Coordinates Received!'; const deep = `itoolsbt://jumpLocation?lat=${latF}&lng=${lngF}`; if (!NTFY_TOPIC) return logInfo(`[NTFY disabled] ${latF},${lngF} ${title}`); try { const headers = { Title: title, Priority: 'high', Tags: 'iphone,arrow_forward,white_check_mark', Click: deep, 'X-Deep-Link': 'true' }; if (pokemonName && pokemonNameToIdMap.has(pokemonName)) { const pokemonId = pokemonNameToIdMap.get(pokemonName); headers.Icon = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${pokemonId}.png?v=${Date.now()}`; } await fetch(`https://ntfy.sh/${NTFY_TOPIC}`, { method: 'POST', body: `${latF},${lngF}`, headers: headers }); logInfo(pokemonName ? `✔ ntfy sent for ${pokemonName}` : '✔ ntfy sent (Coordinates)'); } catch (e) { logError('✘ ntfy failed:', e); } }

function handleWsMessageUpdate(pkt) { try { if (pkt.channel_id !== TARGET_CHANNEL_ID) return; const flags = Number(pkt.flags ?? 0); if (flags !== 0 && (flags & 64) === 0) return; const coords = extractCoords(flattenGatewayPkt(pkt)); if (coords) { const pokemonName = clickedPokemonQueue.shift(); if (pokemonName) { logInfo(`[SUCCESS] Coords for ${pokemonName} → ${coords[1]},${coords[2]}`); sendNotification(coords[1], coords[2], pokemonName); } else { logWarn('[WARN] Got coordinates, but the Pokémon queue was empty.'); sendNotification(coords[1], coords[2], null); } } } catch (err) { logError('[handleWsMessageUpdate] error:', err); } }
